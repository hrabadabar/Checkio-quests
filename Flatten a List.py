...
series of numbers and as the result of his efforts, a simple sequence of 
...
contain yet more lists and integers which then… you get the idea. 
...
We need to hide this program from Nikola by keeping it small and easy to 
hide. Because of this, your code should be shorter than 140 characters (with whitespaces).
...
def flat_list(array):
    def flat(array,seq):
        for val in array:
            if isinstance(val,list): # selection is not an integer
                val = flat(val,seq)  # recur
            else:
